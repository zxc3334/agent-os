...
- Linux root 用户下 claude code 报 `--dangerously-skip-permissions cannot be used with root/sudo privileges`；codex 的 `--yolo` 同理被拒 → 原因是 claude/codex 出于安全考虑拒绝在 root/sudo 下跳过全部权限确认 → 正确做法：claude 改用 `--allowedTools` 工具白名单（如 `--allowedTools Bash,Read,Write,Edit,Glob,Grep,...`），codex 改用 `--sandbox danger-full-access`；注意 codex 的 `--sandbox` 参数不被 `resume` 子命令接受（resume 的权限模式继承自原会话）。长期更优方案是用非 root 用户运行 agent-os，则两个 CLI 都能用回原生跳过权限模式
